...
- Документовано в офіційній специфікації `MODbus Communication Protocol_15-16S-1.pdf`:
  - **Підтверджені журнали запиту:** `10 03 00 00 00 27 06 91` для `Pack16` / адреси `0x10` та `04 03 00 00 00 27 05 85` для адреси `0x04`. Обидва читають 39 регістрів від `0x0000`; адресу вибирає DIP COMBOX.
...
- Прошивка передає раз на 500 мс кадри `0x351`, `0x355`, `0x356`, `0x35C`, `0x35E` у little-endian форматі.
- Також приймається запит інвертора `0x305`; COMBOX відповідає актуальним набором у найближчому дозволеному 500-мс циклі, не порушуючи мінімальний інтервал Deye.
- У `0x35C` специфікація Deye визначає лише біт `0x20` Full Charge Enable. Vision BMS не передає відповідного підтвердженого запиту, тому COMBOX тримає весь кадр нульовим. `0x35E` містить 8-байтовий ідентифікатор сумісності `PYLON   `.
- Кадри `0x359` та `0x35A` свідомо не передаються: їхні аварійні біти не можна безпечно вгадувати з Vision Modbus.
