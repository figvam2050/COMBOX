...
## Verified BMS Communication (39 Registers Map)
- Перевірено та підтверджено утилітою `BMS_TOOLS_2.2.01` на реальній батареї Vision V-LFP48100:
  - **Запит:** `10 03 00 00 00 27 06 91` (Slave Address `0x10`, Read 39 registers).
  - **Відповідь:** `10 03 4E 13 3F 00 00 ...` (83 байти: заголовок + 78 байт даних + 2 байти CRC16).
- **Мапінг регістрів у прошивці:**
  - `Reg 0` (байти 3-4): Загальна напруга пака (0.01 В, наприклад `0x133F` = 49.27 В).
  - `Reg 1` (байти 5-6): Струм пака (0.01 А, signed: >0 розряд, <0 заряд).
  - `Reg 2..17` (байти 7..38): Напруги комірок 1..16 у мВ (для 15S пака комірка 16 = 0 мВ).
  - `Reg 18..21` (байти 39..46): 4 датчики температури (°C) — MOSFET/PCB (23°C), датчик комірок 1 (22°C), датчик комірок 2 (22°C), температура довкілля (15°C).
  - `Reg 22` (байти 47-48): **State of Charge (SOC)** = 100%.
  - `Reg 23` (байти 49-50): **State of Health (SOH)** = 100%.
  - `Reg 24` (байти 51-52): Кількість циклів / залишкова ємність = 31.
  - `Reg 25..27` (байти 53..58): Статус, тривоги та прапорці захисту.
  - `Reg 37` (байти 77-78): Максимальний ліміт струму заряду/розряду (`0x03E8` = 1000 -> 100.0 А).
- **Трансляція в Deye CAN:**
  - Знак струму інвертується для Pylontech 0x356 (у Vision розряд $> 0$, у Deye розряд $< 0$).
  - Середня температура комірок транслюється в 0x356.
  - Ліміти струму та напруги формуються в 0x351.
