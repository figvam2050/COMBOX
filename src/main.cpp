...
/*
  ...
  2) The stock Felicity bootloader/application offset (0x08004000) must be handled
     in the linker script / PlatformIO configuration, NOT in main.cpp.
  3) Hardware pin mapping must still be verified on the actual COM-Box PCB.
*/
...
void USART2_WriteBuffer(const uint8_t *data, uint16_t len) {
  for (uint16_t i = 0; i < len; i++) USART2_Write(data[i]);
}
...
bool sendCANMessage(uint16_t id, const uint8_t *data, uint8_t len, uint32_t timeoutUs = 5000) {
  ...
  while ((micros() - start) < timeoutUs) {
    if (CAN1->TSR & CAN_TSR_TME0) { mailbox = 0; break; }
    if (CAN1->TSR & CAN_TSR_TME1) { mailbox = 1; break; }
    if (CAN1->TSR & CAN_TSR_TME2) { mailbox = 2; break; }
  }
  ...
  uint32_t tmeMask = (mailbox == 0) ? CAN_TSR_TME0 :
                     (mailbox == 1) ? CAN_TSR_TME1 : CAN_TSR_TME2;

  while ((micros() - start) < timeoutUs) {
    if (CAN1->TSR & tmeMask) return true;
  }
  return false;
}
...
void sendBmsRequest(uint8_t address) {
  ...
  while (USART2_Available()) (void)USART2_Read();
...
// -----------------------------------------------------------------------------
bool parseBmsResponse(const uint8_t *buf, uint16_t len, uint8_t expectedAddress, uint8_t slot) {
  if (slot >= MAX_BATTERIES || len != BMS_RESPONSE_LEN) return false;
  ...
  if (bat.soc > 100 || bat.soh > 100) return false;
  if (bat.totalVoltageRaw < 3000 || bat.totalVoltageRaw > 6500) return false;
...
bool chargeAllowedByProtection(uint16_t p) {
  const uint16_t deny =
      PROT_PACK_OV | PROT_CELL_OV | PROT_CHARGE_OC |
      PROT_ENV_TEMP | PROT_MOS_TEMP |
      PROT_CHARGE_OT | PROT_CHARGE_UT;
  return (p & deny) == 0;
}

bool dischargeAllowedByProtection(uint16_t p) {
  const uint16_t deny =
      PROT_PACK_UV | PROT_CELL_UV | PROT_DISCHARGE_OC |
      PROT_ENV_TEMP | PROT_MOS_TEMP |
      PROT_DISCHARGE_OT | PROT_DISCHARGE_UT |
      PROT_DISCHARGE_SC;
  return (p & deny) == 0;
}
...
void buildFrame359(uint16_t warning, uint16_t protection, uint8_t out[8]) {
  ...
  if (protection != 0) out[0] = 0x01; // generic protection present
  if (warning != 0)    out[2] = 0x01; // generic warning present
...
void sendDeyeCanFrames() {
  ...
  if (current01A32 > 32767) current01A32 = 32767;
  if (current01A32 < -32768) current01A32 = -32768;
  int16_t current01A = (int16_t)current01A32;

  // Dynamic high-SOC taper. Never increase BMS limit, only reduce it.
  if (soc >= 99) {
    chargeLimitA = min<uint32_t>(chargeLimitA, 5);
  } else if (soc >= 97) {
    chargeLimitA = min<uint32_t>(chargeLimitA, 10);
  } else if (soc >= 95) {
    chargeLimitA = min<uint32_t>(chargeLimitA, 20);
  }

  if (!chargeEnable) chargeLimitA = 0;
  if (!dischargeEnable) dischargeLimitA = 0;
  ...
  uint8_t d351[8] = {
    (uint8_t)(CHARGE_VOLTAGE_LIMIT_01V & 0xFF),
  ...
  // 0x355: SOC, SOH
  uint8_t d355[4] = {
    (uint8_t)(soc & 0xFF), (uint8_t)(soc >> 8),
  ...
  uint8_t d35C[8] = {0};
  if (chargeEnable)    d35C[0] |= 0x80;
  if (dischargeEnable) d35C[0] |= 0x40;
...
uint8_t getAddressForSlot(uint8_t slot) {
  if (USE_MASTER_ONLY) return MASTER_ADDRESS;
  ...
  if (!waitingForResponse) {
    ...
    while (USART2_Available()) {
      ...
      if (rxIndex == BMS_RESPONSE_LEN) {
        if (parseBmsResponse(rxBuffer, rxIndex, currentExpectedAddress, currentBatteryIndex)) {
        ...
        currentBatteryIndex++;
        if (currentBatteryIndex >= activeSlotCount()) currentBatteryIndex = 0;
        break;
      }
    }

    if (waitingForResponse && (now - responseStartTime > BMS_RESPONSE_TIMEOUT_MS)) {
      waitingForResponse = false;
      currentBatteryIndex++;
      if (currentBatteryIndex >= activeSlotCount()) currentBatteryIndex = 0;
    }
